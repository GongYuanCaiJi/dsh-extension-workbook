...
export async function workbookDoctorReport(ooxml: EngineCapabilities): Promise<WorkbookDoctorReport> {
  ...
  // dsh port: @firstpick/pi-utils is bundled into dist/ at build time (its
  // upstream package ships .ts sources and Node refuses type-stripping under
  // node_modules), so it is no longer a resolvable runtime dependency. This
  // check now reflects the actual runtime dependencies of the port.
