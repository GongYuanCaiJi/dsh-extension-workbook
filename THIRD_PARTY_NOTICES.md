...
shasum firstpick-pi-extension-workbook-0.1.5.tgz   # == 2a2f92cad262afd143d88083574aaf9b970b715d
```

(npm's registry "shasum" field is SHA-1, so the command is `shasum`, not `shasum -a 256`;
the tarball npm packs for a scoped package is named `firstpick-pi-extension-workbook-0.1.5.tgz`.)
